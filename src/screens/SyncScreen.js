...
export default function SyncScreen({ navigation }) {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            const pullRes = await pullServerChanges();

            if (pushRes.success && pullRes.success) {
                Alert.alert('Sincronização Concluída', `Nuvem atualizada com sucesso!\n\nEnviados: ${pushRes.pushed}\nRecebidos: ${pullRes.pulled}`);
            } else {
                Alert.alert('Aviso de Sync', 'Alguns itens falharam. Verifique internet.');
            }
        } catch (error) {
            Alert.alert('Erro Supabase', 'Falha fatal na sincronização.');
        } finally {
            setIsSyncing(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
                ...
                />
                <SettingCard
                    icon="sync-outline" label="Nuvem & Backup Local" description={isSyncing ? "Sincronizando..." : "Conectar com Master Central (Supabase)"}
                    onPress={isSyncing ? null : handleSync}
                />
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView style={styles.modalBody}>
                            ...
                            <Text style={[styles.inputLabel, { marginTop: 15 }]}>PESO PADRÃO POR CAIXA (KG)</Text>
                            <TextInput
                                style={[styles.input, { borderColor: theme.primary_color, backgroundColor: '#ECFDF5' }]}
                                value={settings.peso_padrao_caixa ? String(settings.peso_padrao_caixa) : '20'}
                                keyboardType="numeric"
                                placeholder="Ex: 20"
                                onChangeText={(t) => setSettings({ ...settings, peso_padrao_caixa: t })}
                            />
                            ...
                            <TouchableOpacity
                                ...
                                onPress={async () => {
                                    ...
                                    await updateAppSetting('peso_padrao_caixa', settings.peso_padrao_caixa ? parseFloat(settings.peso_padrao_caixa) : 20);
