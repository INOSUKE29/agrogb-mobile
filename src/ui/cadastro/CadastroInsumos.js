...
import { View, Text, StyleSheet, SectionList, FlatList, TouchableOpacity, Modal, TextInput, Alert, ActivityIndicator, ScrollView, Platform, Image } from 'react-native';
import { v4 as uuidv4 } from 'uuid';
import * as ImagePicker from 'expo-image-picker';
import { insertCadastro, getCadastro, deleteCadastro, updateCadastro, insertReceita, getReceita, deleteItemReceita, insertCadastroMidia, getCadastroMidia } from '../../database/database';
import { LinearGradient } from 'expo-linear-gradient';
import { Ionicons, MaterialCommunityIcons } from '@expo/vector-icons';

// --- CONFIGURAÇÃO DE CATEGORIAS (UX) ---
const CATEGORIES = {
    DEFENSIVO_QUIMICO: { label: 'Defensivo Químico', icon: 'flask-outline', color: '#DC2626', bg: '#FEE2E2', fields: ['principio', 'classe'] },
    DEFENSIVO_BIOLOGICO: { label: 'Defensivo Biológico', icon: 'bug-outline', color: '#16A34A', bg: '#DCFCE7', fields: ['principio', 'classe'] },
    ...
    // FALLBACK SAFETY
    UNKNOWN: { label: 'Outro', icon: 'help-circle-outline', color: '#9CA3AF', bg: '#F3F4F6' }
};
...
export default function CadastroInsumos({ navigation }) {
    ...
    const handleCamera = async () => {
        try {
            ...
            if (status !== 'granted') return Alert.alert('Erro', 'Precisamos da câmera!');
        ...
        } catch (e) { Alert.alert('Erro', 'Falha na câmera: ' + e.message); }
    };
    ...
    // UX Helpers
    const getCategoryConfig = (t) => CATEGORIES[t] || CATEGORIES['UNKNOWN'] || CATEGORIES['INSUMO'];
                ...
                <View style={styles.overlay}>
                    <View style={styles.modal}>
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            {/* 1.5 FOTO DO PRODUTO (NOVO) */}
                            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                                <TouchableOpacity onPress={handleCamera} style={styles.photoBox}>
                                    {fotoUri ? (
                                ...
                                {fotoUri && (
                                    <TouchableOpacity onPress={() => setFotoUri(null)} style={{ marginTop: 5 }}>
                                        <Text style={{ color: '#EF4444', fontSize: 12, fontWeight: 'bold' }}>Remover Foto</Text>
                                    </TouchableOpacity>
                                )}
                            </View>

                            {/* 2. DADOS BÁSICOS */}
                            <Text style={styles.label}>IDENTIFICAÇÃO (Nome/Descrição)</Text>
                            <TextInput
                                style={styles.input}
...
const styles = StyleSheet.create({
    ...
    // Photo Styles
    photoBox: { width: 120, height: 120, borderRadius: 20, backgroundColor: '#F3F4F6', justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#E5E7EB', borderStyle: 'dashed', overflow: 'hidden' },
    photoPreview: { width: '100%', height: '100%' },
    photoLabel: { fontSize: 10, color: '#9CA3AF', fontWeight: 'bold', marginTop: 5 }
});
