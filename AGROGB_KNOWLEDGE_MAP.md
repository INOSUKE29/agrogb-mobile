...
## 13. A Finalização do Portal do Agrônomo (Sessão 2026-06-11)
- **Status Geral:** As telas de "Meu Consultor" (Portal do Agricultor), "Recomendações" e "Visitas Técnicas" (Portal do Agrônomo) foram desenhadas em React (padrão Diamond Pro) para Desktop e o motor de retaguarda (`agronomistService.ts`) foi construído. As telas ainda precisam ter os estados conectados ao Motor amanhã.

### Regras de Ouro Reforçadas (Nunca Mais Burladas):
1. **Governança de Nomenclatura SQL:** Novas tabelas que servem como entidades operacionais diárias pertencem à "Classe 02". Para adicioná-las, NÃO SE CRIA uma classe raiz nova (ex: `13_...`), cria-se um script estendido (ex: `02_A_technical_visits.sql`).
2. **Tenant Isolation (O Pilar Mestre):** Nenhuma tabela operacional nasce no banco de dados sem a coluna `organization_id UUID REFERENCES public.organizations(id)`. Sem isso, as fazendas vazariam informações.
3. **RLS Centralizado e de Alta Performance:** Políticas de RLS não devem ficar espalhadas. Todas pertencem obrigatoriamente ao arquivo `SQL/Master_Scripts/04_agrogb_rls_policies.sql`. Mais ainda: NUNCA usar `auth.jwt() -> 'app_metadata' ->> 'role'` na raiz do RLS. Usar a sintaxe blindada `((select auth.jwt())->> 'user_role')` e `((select auth.jwt())->> 'organization_id')::uuid` para forçar o Postgres a cachear o contexto de segurança usando `initPlan`, o que é O(1).
4. **Isolamento de Lógica Monorepo:** O Frontend (`apps/desktop/src/...` e `apps/mobile/...`) é APENAS "pintura". Todo o código que bate no banco de dados Supabase via `insert()`, `update()`, etc., DEVE ser construído exclusivamente dentro de `packages/services/src/`. O `agronomistService.ts` foi construído e exportado respeitando isso.
5. **Triggers Blindados vs Client-Side Logic:** Operações críticas que envolvem mexer em múltiplas tabelas (ex: ao digitar o código de convite, é preciso inativar o convite E criar a relação na tabela de vínculos) NUNCA podem ser feitas enviando requisições paralelas pelo React. Devem ser empacotadas no servidor via Postgres RPC (`SECURITY DEFINER`), como foi feito em `accept_agronomist_invite()`.
