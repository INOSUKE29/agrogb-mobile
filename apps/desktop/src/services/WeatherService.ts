...
const CACHE_KEY = '@agrogb_weather_data_desktop';
    ...
    fetchHyperlocalWeather: async (lat: number, lon: number): Promise<DadosClima> => {
        try {
            ...
            const mockData: DadosClima = {
                temperaturaAtual: 24 + Math.random() * 5,
                temperaturaMinima: 18,
                temperaturaMaxima: 29 + Math.random() * 2,
                sensacaoTermica: 25,
                umidadeRelativa: 65 + Math.random() * 20,
                probabilidadeChuva: 15 + Math.random() * 40,
                volumeChuvaMmMagnitude: Math.random() * 10,
                velocidadeVento: 8 + Math.random() * 15,
                rajadasVento: 12 + Math.random() * 20,
                indiceUV: 6,
                nascerSol: '06:15',
                porSol: '18:05',
                descricao: 'Variável',
