        ...
        <div className="animate-fade-in pb-12 space-y-8">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-[var(--color-muted)] text-lg max-w-xl">
                            Gestão de fornecedores e aprovação de orçamentos para compra de insumos.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="glass-card rounded-3xl overflow-hidden flex flex-col h-[600px]">
                        <div className="p-6 border-b border-[var(--color-border)] flex items-center justify-between bg-white/[0.02]">
                            ...
                            <button 
                                ...
                                className="bg-[var(--color-primary)]/10 hover:bg-[var(--color-primary)]/20 p-2.5 rounded-xl text-[var(--color-primary)] transition-colors shadow-sm"
                            >
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                            {loading ? (
                                ...
                                <div className="text-center text-[var(--color-muted)] flex flex-col items-center justify-center h-full">
                                    ...
                                    <p className="font-bold">Nenhum fornecedor</p>
                                </div>
                            ) : (
                                fornecedores.map(f => (
                                    <div key={f.uuid} className="bg-white/5 border border-[var(--color-border)] p-4 rounded-2xl flex items-center gap-4 hover:border-[var(--color-primary)]/50 transition-all group cursor-pointer hover:bg-white/10">
                                        <div className="w-12 h-12 bg-gradient-to-br from-[var(--color-primary)]/20 to-transparent rounded-xl flex items-center justify-center text-[var(--color-primary)] border border-[var(--color-primary)]/20 shrink-0 group-hover:scale-110 transition-transform shadow-inner">
                                            <Building2 className="w-5 h-5" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h3 className="text-white font-bold truncate text-base">{f.nome_fantasia}</h3>
                                            <p className="text-sm text-[var(--color-muted)] truncate font-medium">{f.email || 'Sem contato de email'}</p>
                                        </div>
                                    </div>
                                ))
                            )}
            ...
            {showFornecedorModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setShowFornecedorModal(false)}></div>
                    <div className="glass border border-[var(--color-border)] rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden relative z-10 animate-fade-in flex flex-col">
                        ...
                        <div className="p-6 border-b border-[var(--color-border)] bg-white/[0.02] flex justify-between items-center">
                            <h2 className="text-xl font-black text-white flex items-center gap-2">
                                <Store className="text-[var(--color-primary)] w-6 h-6" />
                                Novo Fornecedor
                            </h2>
                            <button onClick={() => setShowFornecedorModal(false)} className="text-[var(--color-muted)] hover:text-white transition-colors">&times;</button>
                        </div>
                        <form onSubmit={handleSaveFornecedor} className="p-6 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-[var(--color-muted)] mb-2 uppercase tracking-wider">Nome / Razão Social *</label>
                                <input 
                                    type="text" required value={nomeFantasia} onChange={e => setNomeFantasia(e.target.value)}
                                    className="w-full bg-[var(--color-background)] border border-[var(--color-border)] text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-[var(--color-primary)] outline-none transition-all"
                                    placeholder="Ex: Agro Insumos S.A"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-[var(--color-muted)] mb-2 uppercase tracking-wider">Telefone</label>
                                <input 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-[var(--color-muted)] mb-2 uppercase tracking-wider">E-mail</label>
                                <input 
                            ...
                            <div className="flex gap-3 pt-4 border-t border-[var(--color-border)]">
                                <button type="button" onClick={() => setShowFornecedorModal(false)} className="flex-1 py-3 text-[var(--color-muted)] hover:bg-white/5 rounded-xl font-bold transition-all">Cancelar</button>
                                <button type="submit" className="flex-1 py-3 bg-[var(--color-primary)] rounded-xl text-white font-black shadow-lg shadow-[var(--color-primary)]/20 active:scale-95 transition-all">Cadastrar</button>
                            </div>
                        </form>
                    </div>
                </div>
